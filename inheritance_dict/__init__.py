...
class InheritanceDict(dict):
    ...
    def get(self, key, default=None):
        ...
        If `key` is a type, the lookup walks the type's MRO (including the type itself) and returns the first matching value; for non-type keys a direct lookup is attempted. If no candidate is found, `default` is returned.
        """
        try:
            return self[key]
        except KeyError:
    ...
    def setdefault(self, key, default=None):
        ...
        This method uses the same lookup semantics as __getitem__: if `key` is a type, the mapping is searched along the key's MRO and the first matching value is returned. If no mapping is found, `default` is stored under the exact `key` provided (no MRO walking when writing) and `default` is returned.
    ...
    def __repr__(self):
        ...
        Returns:
            str: A representation in the form "InheritanceDict(<dict-repr>)", where <dict-repr> is the underlying dict's repr.
        """
